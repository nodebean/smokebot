...
@hook.command("qa")
@hook.command
def add_smokedot_quote(inp, nick="", db=None):
    quote = re.match(r"[^\w@]+(\S+?)>?\s+(.*)", inp, re.I)
    quoted_nick, msg = quote.groups()
    try:
        add_quote(db, "#smokedot", quoted_nick, nick, msg)
        db.commit()
    except db.IntegrityError:
        return "message already stored, doing nothing."
    return "quote added."
...
@hook.command("rq")
@hook.command
def get_random_smokedot_quote(inp, db=None):
    quotes = get_quotes_by_chan(db, "#smokedot")
    n_quotes = len(quotes)
    num = random.randint(1, n_quotes)
    selected_quote = quotes[num - 1]
    return format_quote(selected_quote, num, n_quotes)
